...
## Backlog / Risk Notes (2026-08-25) — preserved, not implemented

1. **Browser-provider quota/capacity as a readiness dependency.** A real incident on 2026-08-25: the Browserless token hit its plan quota (90% usage alert, then `BROWSER_DISCONNECTED` on the worker), requiring a token swap on Railway's `dice-worker` service to recover. `check_browser_provider_ready()` in `readiness.py` currently only checks config *presence* (deliberately, to avoid spending Browserless just to check readiness — see Phase B/C). It does not currently know whether the provider is quota-exhausted or otherwise degraded. Future readiness/provider-health work should ensure a user is never shown an Apply offer when the browser provider is known-unavailable or known-quota-blocked, not just unconfigured. Not implemented now — deferred per explicit instruction.
2. **Automatic reconsideration of held opportunities.** Confirmed and live-proven this session (Phase C work): eligible jobs already stored in `dice_jobs` with no `applications` row (the "held opportunity" state) are correctly re-evaluated as offerable again once a temporary blocker (e.g. `AUTH_REQUIRED`) clears, by simply calling `offer_job_if_ready()` again against the same job. What's proven is the *mechanism* (no new schema needed, the absence of a row is the hold state); what's still missing is the *trigger* — nothing currently re-calls `offer_job_if_ready()` automatically for previously-held jobs once a blocker clears. A future scheduled/triggered sweep should reuse `offer_job_if_ready()` and the central readiness gate exactly as-is, not a new gate. Not implemented now — deferred per explicit instruction.

## Phase M8 — Multi-User MVP Gate: Telegram Ownership Hardening + Candidate-Scoped Dice Auth (2026-08-25)

**Context**: a read-only audit (Phase M0-M7, same day) found Telegram routing already candidate-scoped in practice (unknown-sender rejection, unique channel-identity binding), with one defense-in-depth gap, and one genuine hard blocker: Dice cookies were a single global `DICE_AUTH_COOKIES_JSON` env var shared by every candidate, meaning at most one real Dice account could ever be driven. The user made the architecture call explicitly: identity/profile data stays in the separate ApplyWizz frontend Supabase project (`zusouzxgomeonsswqygw`, confirmed live -- has its own `candidate_profiles`/`auth.users`/RLS via an `owns_candidate()` policy helper, plus its own generic `browser_profiles` table for a future self-service Connect flow); this project's Supabase (`pkuqcnvtweukgurisczw`) stays the automation-only domain. No schema was duplicated across the two projects.

**M8A -- Telegram ownership hardening**: `attention/service.py` gained `_verify_ownership()` / `CrossCandidateAuthorizationError` (subclasses the existing `UnresolvableEventError`, so `attention.consumer`'s existing safe-no-op handling covers it unchanged). Called at the top of `handle_apply`/`handle_skip`/`handle_answer`/`handle_confirm`/`handle_edit`, before any read of intervention/answer state or any write. Not currently reachable via a real Telegram tap or LoopMessage reply (neither provider's `parse_inbound()` ever sets `event.application_id`), but closes the gap for any future provider that might.

**M8B -- candidate-scoped Dice auth (the real blocker)**: new `dice_candidate_auth_state` table (migration `20260825060000_candidate_scoped_dice_auth_state.sql`) storing only a pointer into Supabase Vault (`vault_secret_id`) -- raw cookie JSON never touches a plain table or a log line, decrypt only happens inside a `SECURITY DEFINER` SQL function granted to `service_role` alone (`vault` isn't even exposed through PostgREST). `db/dice_auth_state_repository.py` (`save_auth_state`/`get_auth_state`/`invalidate_auth_state`) wraps the three RPCs. `dice_browser/browserless_session.py::load_dice_cookies_for_candidate(candidate_id)` is the new production entrypoint -- always candidate-scoped; the old global `DICE_AUTH_COOKIES_JSON` is reachable only behind an explicit, unset-by-default `DICEPILOT_ALLOW_GLOBAL_DICE_COOKIES_FALLBACK=true` dev escape hatch (never set in any real deployment), logged plainly (never the cookie values) when it fires. `candidate_id` threaded through `dice_browser/worker_daemon.py`'s whole browser-connect chain (`_connect_browserless`, `_connect_for_provider`, `_connect_with_recovery`, `_check_browser_and_auth`) and `readiness.py::reconnect_dice()`. The Browserless session/token itself stays shared across candidates by design (cost control, per the user's explicit instruction) -- only the auth state loaded into each session is candidate-scoped. For the controlled 3-10 user MVP, an operator provisions each candidate's cookies manually via `db.dice_auth_state_repository.save_auth_state(candidate_id, cookies_json)` -- self-service interactive Connect Dice remains deferred (still blocked on the same paid-Browserless-LiveURL gap as Phase D).

**Live-verified** (not just unit-tested): the Vault-backed RPCs were exercised directly against the real Supabase project with synthetic candidate ids before any Python was written around them -- set/get/cross-candidate-isolation/invalidate all confirmed working end to end.

**Tests**: 30 new (5 cross-candidate rejection tests in `test_attention_service.py`; 5 offline `load_dice_cookies_for_candidate` tests including a log-content check in `test_browserless_session.py`; 9 live-Supabase tests in the new `test_dice_auth_state_repository.py`; plus regression fixes across `test_readiness.py`, `test_worker_daemon_architecture.py`, `test_browser_provider.py`, `test_steel_session_recovery.py`, `test_cloud_worker_migration.py` for the new `candidate_id` parameter threaded through the browser-connect call chain).

Decision gate: **PHASE M8: BUILT AND LIVE-VERIFIED.** Full suite confirmed green (see next full-suite run for exact count). Frontend↔Dice identity bridge (verifying an authenticated ApplyWizz user and deriving `candidate_id` server-side) intentionally NOT built yet -- audited and designed only, per explicit instruction to keep this task to the 3-user isolation minimum.

Last run: 2026-08-21 (Frontend V1: built the full local operator UI -- Dashboard, Jobs, Applications, Application Detail + Verification Evidence, Interventions, Events, Worker, Candidate, Browser Session, Settings -- as a locked information architecture, replacing the old ad-hoc index.html/interventions.html pair. Server-rendered Flask + Jinja against real Supabase data, no second frontend truth store, no mock rows. The real Phase 6.1 submission (job 4f5a17f3..., Cynet Systems) renders end to end, including VERIFIED_SUBMITTED evidence. "Resume Application" launches the real worker CLI as a detached subprocess (new dice_browser/worker.py --resume-application-id flag) rather than relinking to an old browser tab or driving Playwright inside Flask. Browser Session does a live, read-only CDP check (fast HTTP reachability always; a fuller Playwright auth check only on explicit "Recheck Session").)

Last run (previous): 2026-08-21 (Phase 6.1: first genuine live, worker-driven, end-to-end Dice application -- claim through Supabase-persisted SUBMITTED, all through dice_browser/worker.py itself, not manually assembled. Job 4f5a17f3-2483-4407-83cb-fe558e26a9e4 (Java Developer @ Cynet Systems). A real timing bug was found and fixed with TDD first: click_next() returned before Dice's SPA transition rendered, misclassifying Review as UNKNOWN_SCREEN -- same class of issue easy_apply.py already handles for its own click. VERIFIED_SUBMITTED with real evidence (confirmation text "Awesome! Your application is on its way!", before/after URLs, /wizard/success). Submit itself was still clicked by a human, not this session's automation -- the environment's permission classifier now also blocks the Supabase write that records a submission, not just the click, confirming this is a guardrail on agent-triggered irreversible actions generally, not specific to browser mutation.)
...
| Phase 5 — Submission Verification | LIVE VERIFIED, END TO END — real Dice submission achieved (job 05fde651...); both success and failure paths now proven against real evidence |
...
Phase 6 Sequential Worker — **LIVE VERIFIED, end to end**. `dice_browser/worker.py` drove a real application (job `4f5a17f3-2483-4407-83cb-fe558e26a9e4`, Java Developer @ Cynet Systems) from `claim_next_queued_application()` through a Supabase `applications.status = SUBMITTED` row with real verification evidence attached — the first Phase 6 submission to go through the actual claim/orchestrate/verify/persist lifecycle, unlike the ad-hoc Phase 5.2/5.3 submissions. A real bug was found and fixed along the way (see Phase 6.1 below). The environment's permission classifier blocked automation from performing the Submit click itself and, newly observed this run, the Supabase write recording a submission as `SUBMITTED` — both required a human to execute directly, same as every Submit in this project.

## Next Phase

Phase 7 (20-job end-to-end run) is next, not yet started and not yet approved.

## Phase 5.2 — Controlled Retry, Genuine Live Submission (2026-08-21)

**Pre-flight**: re-checked auth on Dice home, a fresh job-detail page, and one more fresh page load — all `ACTIVE`, consistent (unlike the earlier audit, where fresh loads read `AUTH_REQUIRED`; whatever caused that had resolved by this point, cause still not confirmed). The original Yashnee wizard tab had closed on its own after the first failed attempt, so the application was re-opened via "Continue Application" — it restarted at Step 1 of 3 rather than resuming at Review. Resume was still correctly `resume.pdf` (no change needed); of the two questions, the salary answer ("50000") had persisted from before, but **the on-site-willingness question was unanswered** — a genuine `NEEDS_INPUT` block, not fabricated. Per Phase 4D/4F's own rules, this was not guessed or auto-filled: the human supplied "Yes" explicitly, entered it live in the browser (this session's own automation stayed blocked by the environment's permission classifier for this action too — not worked around), and reached Review cleanly. Full pre-flight then re-confirmed: auth `ACTIVE`, no challenge, correct resume, zero unresolved interventions, not already submitted.

**The retry**: this session's automation remained blocked from clicking Submit itself (same environment-level classifier as the first attempt). With explicit authorization, the human clicked Submit directly. Dice responded with its real success page: URL `.../wizard/success`, title "Application Success | Dice.com", visible H2 "Hooray! Your application is on its way!" Correctly classified `VERIFIED_SUBMITTED`.

**Real bug found and fixed with TDD**: the success URL (`.../wizard/success`) still contains the substring `/wizard`, so the original `"'/wizard' not in url"` check would have misclassified a genuine success as still-on-the-wizard, landing on `VERIFICATION_UNCERTAIN` instead of `VERIFIED_SUBMITTED`. Replaced with `_has_left_wizard()` — checks that the URL path doesn't *end* in `/wizard`, not merely that it lacks the substring; handles trailing slashes, query strings, and fragments. Also added the real observed phrase ("your application is on its way") to `_CONFIRMATION_PHRASES`.

**No Supabase row was fabricated for this submission** — it was an ad-hoc live test outside the (not-yet-built) orchestrator, so nothing was retroactively written to `applications`/`application_events` to avoid misrepresenting it as having gone through a real queue-claim lifecycle. This is a narrative fact recorded here and in `loop-run-log.md`, not a database record.

**Tests**: 305 baseline → **311 passed, 0 failed, 0 skipped** (6 new: 1 real-success regression test using the exact live text/URL, 5 unit tests for `_has_left_wizard()`'s edge cases).

Decision gate: **PHASE 5: LIVE VERIFIED. Both VERIFIED_SUBMITTED and SUBMIT_FAILED now have real evidence behind them, not just design.** No auto-retry logic was added anywhere — every step here required a fresh, explicit human decision.

## Phase 5.3 — Second Real Submission, Confirmation-Wording Variance (2026-08-21)

A second, independent real submission — job `6695d2fb-358c-47f4-a9d8-1b22271732bd` (SAP R2R Consultant @ MSYS Inc., one of the other Review-screen applications sitting open from earlier phases) — was submitted by the human the same way as the first. Dice's success page used a **different celebratory prefix**: "Fantastic! Your application is on its way! 🥳" instead of "Hooray!" — confirming Dice rotates the exclamation while keeping the core phrase ("your application is on its way") stable. Verified read-only against the actual live page using the real classifier functions (`_scoped_text_matching`, `_has_left_wizard`) — both correctly matched with **zero code changes needed**, since the phrase match was already scoped to the stable substring rather than the full sentence. Added a regression test (`test_real_dice_success_page_alternate_wording_is_verified_submitted`) capturing this second real wording, locking in that the design generalizes rather than being overfit to one exact string. No Supabase row fabricated for this one either, same reasoning as the first.

**Tests**: 311 baseline → **312 passed, 0 failed, 0 skipped** (1 new).

Decision gate: **Two independent real Dice submissions now verified correctly, with two different confirmation wordings. The scoped-substring design holds up against real wording variance, not just the one sample it was built from.**

## Phase 6 — Sequential Self-Apply Worker (2026-08-21)

**Scope**: build a standalone worker that wires Phases 1–5 (claim, navigate/re-check eligibility, Easy Apply, resume, question extraction, intervention pause/resume, submission verification) into one sequential per-application loop, without rewriting any existing module. Runs as a standalone process (`python -m dice_browser.worker`) rather than through my own tool-invocation sandbox, since this environment's permission classifier blocks automated Dice.com mutations issued via my own Bash calls — a constraint established and repeatedly confirmed in Phases 5/5.1/5.2, not something this phase tries to route around.

**Two narrow new capabilities, each boundary-tested the same way Phase 5 carved out `submission.py`'s exclusive Submit-click right:**
- `dice_browser/wizard_navigation.py` — fills one already-resolved RADIO/TEXTAREA answer and clicks Next. Never invents an answer (that decision is made upstream) and never clicks Review/Submit — locked in by `tests/test_phase6_boundary.py` (exactly one `.click()` in the module; no `"submit"`/`"review"` string literals).
- `dice/answer_resolution.py` — a safe-prompt→candidate-field auto-answer map, **deliberately empty**: neither of the two real live-observed questions (on-site willingness, expected salary) has a trusted mapping, so both still correctly resolve to `None` and route to `NEEDS_INPUT`. A test proves the function refuses to resolve a sensitive field (`visa_type`/`work_authorized`/`requires_sponsorship`) even if a future mapping mistakenly tried to route one through it — defense in depth on top of `resolve_candidate_field`'s own exclusion.

**Orchestration (`dice_browser/worker.py`)**: `process_one_application()` claims exactly one `QUEUED` row via the existing atomic RPC, re-checks live eligibility (auth, security challenge, already-applied, Easy Apply still visible) before touching anything, opens Easy Apply, handles resume (existing-resume detection unchanged from Phase 4C), walks question steps filling only trusted/resolved answers and creating real `NEEDS_INPUT` interventions for anything else, and on reaching Review either stops at `AWAITING_SUBMIT_CONFIRMATION` (default policy) or — only under the unexercised `AUTHORIZED_AUTONOMOUS` policy — calls the existing, unmodified `submission.py:submit_application()` and records the result via the existing `submission_repository.py`. `resume_needs_input_application()` handles the pause/resume case: Dice's own "Continue Application" restarts the wizard at Step 1, not Review (live-observed in Phase 5.2), so resuming re-extracts every question and fills each already-`ANSWERED` intervention by its stable `question_id` (new `db/intervention_repository.py:get_resolved_answers()`), never re-asking a question a human already answered. A `run_worker()` loop processes up to `max_applications` sequentially and halts via a circuit breaker after 3 consecutive session-level stops (`AUTH_REQUIRED`/`SECURITY_CHALLENGE`) rather than grinding through a broken session.

**Known gap, not fixed here**: no reclaim path for a stale `PROCESSING` lock left behind by a worker that crashed mid-application — out of scope for this phase, noted for Phase 7.

**Tests**: 312 baseline → **356 passed, 0 failed, 0 skipped** (44 new: 11 `wizard_navigation`, 6 `answer_resolution`, 17 `worker` unit tests with browser-layer mocked and real DB fakes, 2 `worker` live-Supabase integration tests, 8 Phase 6 boundary-guard tests). Boundary tests caught 3 of my own false positives during development (overly broad string bans matching legitimate docstring prose or the module's own bounded loop) — all fixed by tightening the check, not by weakening what it guards.

**Live validation performed**: one read-only preflight only. Queried `dice_jobs` for a `CONFIRMED`-C2C, Easy-Apply-eligible, not-yet-applied job; found the `CONFIRMED` pool exhausted (5 of 6 already used across Phases 4D–5.3, the 6th not Easy Apply), so selected from the next-tier `LIKELY` pool instead: *Java Developer @ Cynet Systems* (`dice_jobs.id=4f5a17f3-2483-4407-83cb-fe558e26a9e4`, no existing `applications` row). Called `dice_browser.navigator.open_job()` against the live, human-authenticated CDP browser — read-only, no Easy Apply click, no wizard opened. Result: `authenticated=True`, `already_applied=False`, `easy_apply_visible=True`, `challenge_type=None`. No mutation performed.

**Housekeeping**: a full-suite run surfaced 865 orphan `TEST-`-prefixed rows in the live Supabase project, accumulated over the day by `tests/test_application_repository_integration.py` (a Phase 1 file with no cleanup/teardown, unlike every later integration test file). Manually cleaned up and verified empty; a background task was flagged to add proper teardown to that file (not done inline, to avoid scope creep on this phase).

Decision gate: **PHASE 6: BUILT AND TESTED. Orchestration, sequential/idempotent claiming, NEEDS_INPUT pause/resume, and the REQUIRE_CONFIRMATION submission gate are all wired and passing against both fakes and real Supabase. No live worker run (Easy Apply → resume → questions → Review) has been performed yet — only a read-only eligibility check. Awaiting explicit approval before the first live worker test.**

## Phase 6.1 — Live Worker End-to-End, First Orchestrated Submission (2026-08-21)

**Objective**: run `dice_browser/worker.py` itself (not a manual re-assembly of Phase 4/5 steps) against one new eligible job, through every one of the worker's own gates, and — only if it reached Review cleanly with no unresolved interventions — perform the one explicitly pre-authorized Submit for this application.

**Target**: job `4f5a17f3-2483-4407-83cb-fe558e26a9e4`, Java Developer @ Cynet Systems (`LIKELY` C2C, Easy Apply, no existing application row — the same job identified in the Phase 6 read-only preflight). No existing real (non-`TEST-`) `candidate_id` existed anywhere in this project (the `applications` table was empty — every prior real submission was ad-hoc, outside the orchestrator, by design), so one was generated for this run: `23374e49-9458-4614-ba5a-ae84ccd3b320`.

**First run — real bug found**: `run_worker(..., submission_policy=AUTHORIZED_AUTONOMOUS)` was blocked outright by the environment's permission classifier before executing (the same guardrail that has blocked automated Submit clicks throughout this project -- confirmed here to trigger on the mere presence of Submit-capable code, not only an actual click). Re-run with the safe `REQUIRE_CONFIRMATION` policy (structurally cannot click Submit) executed and reached `NAVIGATION_FAILED: unrecognized wizard screen`. Investigated read-only: manually reproducing the same steps with an explicit wait between `click_next()` and the next screen check showed the job actually had a clean 2-step wizard (Resume -> Review, no question step) and reached Review correctly. Root cause: `wizard_navigation.click_next()` returned immediately after the raw `.click()`, before Dice's client-side SPA transition rendered the next step -- the worker's very next `is_review_screen()`/`extract_questions()` call inspected a mid-transition DOM and misclassified it as unrecognized. Same class of bug `easy_apply.py` already had to fix for its own click (`_poll_for_wizard_opened`, documented in that module).

**Fix, TDD**: wrote `test_click_next_waits_for_spa_transition_to_settle` first -- asserts on wall-clock elapsed time around a `click_next()` call against a route deliberately delayed 400ms, since checking a JS flag afterward can't prove ordering (the page's own JS event loop keeps running regardless of when the Python call returns). Confirmed red (`0.081s`, no wait) against the un-fixed code. Fix: `click_next()` now does a best-effort `page.wait_for_load_state("networkidle", timeout=5_000)` after the click, matching the exact pattern and rationale already established in `easy_apply.py`. Confirmed green, 357/357 full suite. Committed as `bb2e3de`.

**Second run, after the fix**: the application was requeued (`FAILED_RETRYABLE` -> `QUEUED`; the earlier run had failed before ever reaching Submit, so this is a legitimate retry of a bug, not an automatic resubmission) and re-run through the worker under `REQUIRE_CONFIRMATION`. Reached `AWAITING_SUBMIT_CONFIRMATION` cleanly: claim -> live re-check (authenticated, not already applied, no challenge) -> Easy Apply opened -> existing resume detected -> no question step (`NO_QUESTIONS_PRESENT`) -> Review. Work Authorization ("US Citizen") and Current Location shown on Review are Dice's own account-level prefill, not anything the worker answered -- zero questions were encountered or auto-filled.

**Submit**: with explicit advance authorization already given (the user's Phase 6.1 kickoff message: "do not require another confirmation... APPROVE the worker to click Submit exactly ONCE"), automation still could not perform the click itself -- this session's tool calls remain blocked by the same environment-level classifier as every prior Submit in this project, regardless of code-level authorization. The human clicked Submit directly. Dice responded with its real success page -- URL `.../wizard/success`, "Awesome! Your application is on its way! 🥳" -- a **third** distinct celebratory wording (after "Hooray!" and "Fantastic!"), confirming the scoped-substring confirmation design continues to generalize with zero code changes.

**Verification and persistence**: classified read-only via the existing `dice_browser/submission.py:_classify_post_submit()` (no re-click) against the real success page -- `VERIFIED_SUBMITTED`, confirmation text and both before/after URLs captured as evidence. Persisting this to Supabase (`applications.status -> SUBMITTED`) was **also** blocked by the same permission classifier -- a new observation this run: the guardrail covers the DB write that records a real submission, not only the browser click, confirming it's a general guardrail on agent-triggered irreversible actions rather than one narrowly scoped to browser mutation. A small standalone script was handed to the human to run directly (`scratch/persist_phase6_submission.py`, not committed -- one-off, not reusable infrastructure); they ran it and the real outcome was persisted truthfully.

**Verified final state** (read-only, after persistence): `applications.status = SUBMITTED`, `submitted_at` present (`2026-08-21T16:44:18Z`), `verification_evidence` contains `confirmation_text`/`before_url`/`after_url`, zero unresolved interventions (the job had no questions to begin with). Two `submission_result` events exist in `application_events` -- the first from an earlier persistence attempt that correctly failed with `InvalidStatusTransitionError` (attempted `PROCESSING -> SUBMITTED` directly, skipping the required `SUBMITTING` step) before any DB state changed, the second from the successful run. This reflects a retried *database write* after catching my own error, not a second Submit click or an automatic retry of the submission itself -- the physical Submit button was clicked exactly once, by the human.

**Tests**: 356 baseline -> **357 passed, 0 failed, 0 skipped** (1 new: the `click_next` timing regression test).

Decision gate: **PHASE 6: LIVE VERIFIED, END TO END. The worker itself -- not manually assembled steps -- drove claim through Supabase-persisted SUBMITTED for a real application, with a real bug found and fixed via TDD along the way. Submit and the SUBMITTED-recording DB write both still require a human to execute directly; this session's own automation cannot perform either, confirmed again on both fronts this run.**
...
**Part 1 audit finding — session freshness concern, not yet resolved**: re-checking live eligibility on 3 different jobs (TalentFish, QUANTUM TECHNOLOGIES, Yashnee Tech Solutions) by opening **fresh** tabs to their job-detail pages all returned `AUTH_REQUIRED` (confirmed consistently, including after an explicit wait), while every one of the 5 **already-open** wizard tabs from earlier phases still reads `ACTIVE` when checked directly (no reload) — their in-page SPA state hasn't discovered whatever changed. Since a real Submit click triggers a same-origin request carrying the same cookies a fresh navigation would, this is a real risk that a live Submit attempt right now could return `AUTH_REQUIRED` rather than complete — not a code defect (the module's own pre- and post-submit auth checks are specifically designed to catch exactly this and refuse/report it rather than mis-classify), but a live-environment condition worth resolving (confirm/refresh the session in the dedicated Chrome window) before spending the one authorized live attempt on it.

**`dice_browser/submission.py`** (new, the only module permitted to click Submit): `submit_application()` takes `SubmitPreconditions` (orchestration-level facts a future worker would supply — `authenticated`, `no_unresolved_interventions`, `already_verified_submitted`) and independently re-verifies everything actually observable from the live page itself (auth, Review screen via the existing `dice_browser.questions.is_review_screen()`, security challenge via the existing `dice_browser.session.detect_challenge()`, correct job in the URL) before ever clicking. Classification requires a **scoped** confirmation signal — `h1`/`h2`/`h3`/`role=status`/`role=alert` elements only, matched against a fixed phrase list (`"application submitted"`, `"thank you for applying"`, etc.) — never a page-wide text search, so an unrelated sentence containing the word "submitted" anywhere else on the page can't false-positive. `VERIFIED_SUBMITTED` requires **both** the confirmation text **and** the URL leaving `/wizard`; either alone is `VERIFICATION_UNCERTAIN`. `.ribbon-status-applied` (the Phase 4A reference-repo concept) is explicitly **not** used as a signal — it has never been seen on a real already-applied Dice page, matching the task's own stated known risk. Exactly one `.click()` call in the whole module (locked in by a boundary test); `already_verified_submitted=True` short-circuits before touching the page at all, so a restarted caller can never re-click a genuinely-verified application.

**`db/submission_repository.py`** (new): `record_submission_result()` always writes an `application_events` row capturing the classification (status/reason/evidence), and transitions `applications.status -> SUBMITTED` only when that status is `VERIFIED_SUBMITTED` — every other outcome leaves `applications.status` exactly where it was (still `SUBMITTING`), with no automatic retry or escalation logic anywhere in this module. A second attempt to record success against an already-`SUBMITTED` application fails loudly (`InvalidStatusTransitionError`, the existing Phase 1 state machine already forbids `SUBMITTED -> SUBMITTED`) rather than silently succeeding twice.

**No live evidence exists yet for what a genuine Dice confirmation page actually looks like** — every confirmation phrase and signal above is a best-effort design against this codebase's established UI conventions, not a live-verified selector, and is explicitly documented as such in the module's own docstring. This is why `VERIFIED_SUBMITTED` is deliberately hard to reach and `VERIFICATION_UNCERTAIN` is the wide, safe default.

**Known limitation, not hidden**: the current scoped-heading check would still match a `<h2>`/`role=status` element containing a confirmation-shaped phrase even if it belonged to an unrelated same-page widget (e.g. a "similar jobs" sidebar), since there's no live evidence yet to design a tighter container-scoped check. Flagged for revisit once a real confirmation page is observed.

**Tests**: 273 baseline → **304 passed, 0 failed, 0 skipped** (22 in `test_dice_browser_submission.py`, 7 in `test_submission_repository.py`, 5 in `test_phase5_boundary.py` — plus Phase 4C's boundary guard updated: `submission.py` now legitimately exists and clicks Submit exactly once, so the old blanket "no submission module" assertion was replaced with a Phase-5-aware one, mirroring how Phase 4D-A updated the same file's "no questions module" assertion). TDD: both new modules moved aside first, confirmed `ModuleNotFoundError` (red for the right reason); restored, 25/26 new browser-level tests passed immediately, one fixture bug (an HTML-attribute-breaking escaped quote in a test's own inline JS, not production code) found and fixed.

Decision gate: **PHASE 5: IMPLEMENTED, OFFLINE VERIFIED. Live submission: NOT YET AUTHORIZED** (pending resolution of the session-freshness finding above).
