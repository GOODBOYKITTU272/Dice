"""Jobs selection -> persistent Supabase run (Phase 6.3: jobs_apply writes
a PENDING run and nothing else -- it never launches a worker process of
any kind, so there is no subprocess to mock here; the standalone
dice_browser.worker_daemon, covered separately in
tests/test_worker_daemon_architecture.py, is the only thing that ever
claims and processes a run). Real Supabase for job/application state,
matching this project's established local_app test convention.
"""
...
def test_apply_without_candidate_config_blocks_run(monkeypatch):
...
def test_apply_queues_selected_jobs_and_starts_a_run(configured_candidate):
...
def test_run_contains_only_selected_application_ids(configured_candidate):
...
def test_preexisting_queued_application_not_swept_into_new_run(configured_candidate):
...
def test_apply_does_not_create_duplicate_application_row(configured_candidate):
...
def test_submitted_job_cannot_enter_a_new_run(configured_candidate):
    from db.application_repository import enqueue_application
...
def test_skipped_job_cannot_enter_a_new_run(configured_candidate):
...
def test_run_progress_page_shows_selected_and_current_job(configured_candidate):
...
# 18. Stop Run sets stop_requested -- checked by run_worker_for_run before
# it claims its NEXT application (worker-side coverage lives in
# tests/test_worker_run.py and test_worker_daemon_architecture.py; this
# checks the route + registry wiring). Deliberately does not assert
# status == 'STOPPED' here: only the worker daemon writes that, and only
# once it actually stops between applications -- never this route.
def test_stop_run_route_sets_stop_requested(configured_candidate):
    ...
    try:
        ...
        assert run["status"] == "PENDING"  # untouched by the stop request itself
    finally:
        _cleanup(job["id"])
...
# tests/test_worker_daemon_architecture.py::test_jobs_apply_never_calls_subprocess_popen
# for the dedicated Popen-absence check).
...
def test_jobs_apply_route_never_imports_playwright_or_calls_worker_functions_directly():
    ...
    for forbidden in ("playwright", "sync_playwright", "run_worker_for_run(", "process_one_application(", "subprocess", "popen"):
        assert forbidden not in body, f"jobs_apply must never launch or call the worker directly ({forbidden!r} found)"
